...
if ($fm->user['id'] !== 0 && $fm->input['action'] == 'markall') mark_forum($forum_id);
...
if ($fm->exbb['statvisit']) {
	...
	if ($statviewing || $statforum) viewing($statviewing, $statforum);
}
...
foreach ($keys as $id=>$topic_id) {
		if (!isset($topics[$topic_id]['name']) || $topics[$topic_id]['name'] == '') $topics[$topic_id]['name'] = $topics[$topic_id]['author'].date(" d.m.Y H:i",$topics[$topic_id]['date']);
...
function pinned($list) {
	...
	if (!empty($list['pinned'])) $pinned[] = $list['id'];
	else $keys[] = $list['id'];
}
...
function filtered($word,$topics) {
		...
		foreach ($topics as $id=>$info) {
				if (isset($info[$field]) && preg_match("/$word/i",$fm->_LowerCase($info[$field]))) $res[$id] = $info;
		}
		return $res;
}
