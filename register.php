...
if ($fm->input['action'] == 'addmember') {
	...
	if ($requirepass === TRUE) {
		...
		$email	= sprintf ( $fm->LANG['RegRequest'],
							$fm->exbb['boardname'],
							$fm->exbb['boardurl'],
							$fm->input['inmembername'],
							$fm->input['password'],
							$code,
                            $filehash);
		$fm->_FcloseAll();
        ...
        $fm->_Message($fm->LANG['Registration'],$fm->LANG['RegThanks'].$fm->LANG['PassSended'],'register.php?action=activate');
        	...
        	$fm->LANG['RegThanks'] = sprintf($fm->LANG['RegThanks'],$fm->exbb['boardname']);
			if ($fm->exbb['emailfunctions'] === TRUE) {
				$email	= sprintf ( $fm->LANG['EmailThanksRegistration'],
									$fm->exbb['boardname'],
									$fm->exbb['boardurl'].'/index.php',
									$fm->input['inmembername'],
									$fm->input['password']);
				...
				if ($fm->exbb['newusernotify'] === TRUE) {
					...
					$email		= sprintf($fm->LANG['EmailNewUserRegistered'],
										$fm->exbb['boardname'],
										$fm->exbb['boardurl'].'/index.php',
										$fm->input['inmembername'],
										$fm->input['password'],
										$fm->input['emailaddress'],
										$fm->input['homepage'],
										$fm->_IP);
