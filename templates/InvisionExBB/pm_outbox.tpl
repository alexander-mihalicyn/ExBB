...
function ChekUncheck() {
	MainChek  = (document.delet.chek.checked==true) ? true:false;
...
//-->
			...
			<form action="messenger.php" method="post" name="delet">
				...
				<table class="tableborder" cellpadding="0" cellspacing="1" width="100%">
					...
					<tr class="titlemedium" align="center" valign="middle">
						<td>{$fm->LANG['MessageStatus']}</td>
						<td width="17%">{$fm->LANG['Owner']}</td>
						<td width="50%">{$fm->LANG['MessageTitle']}</td>
						<td width="18%">{$fm->LANG['MessageDate']}</td>
						<td width="8%"><input name="chek" type="checkbox" value="" onClick="ChekUncheck()"></td>
					</tr>
					{$inbox_data}
DATA;
if ($totaloutbox === 0) {
echo <<<DATA
					<tr>
						<td colspan="5" class="row4" style="padding:5px;margin-top:1px" align="center" valign="middle">
...
echo <<<DATA
					<tr>
						<td class="pformstrip" align="right" colspan="5"><input type="submit" value="{$fm->LANG['Delete']}" onClick="return Chekcheked();">&nbsp;</td>
					</tr>
					<tr>
						<td class="activeuserstrip" align="center" colspan="5">&nbsp;</td>
					</tr>
				</table>
			</form>
			<br>
