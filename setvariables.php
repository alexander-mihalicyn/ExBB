...
if (isset($fm->input['save']) && $fm->_POST) save_info();
...
function save_info(){
		...
		foreach ($fm->input['new_exbb'] as $arr_key => $arr_keyarray) {
				if ($arr_key == 'b') {
					...
					foreach ($arr_keyarray as $key => $variable) {
							switch ($key) {
								case 'file_type':		$fm->exbb[$key] = ($variable == '' || $variable == '0') ? '.*': strval(str_replace(' ','',$variable));
														break;
								case 'default_lang':	$fm->exbb[$key] = Check_DefLangSkin('language','default_lang',$variable);
														break;
								case 'default_style':	$fm->exbb[$key] = Check_DefLangSkin('templates','default_style',$variable);
														break;
								default:				$fm->exbb[$key] = ($variable == '' || $variable == '0') ?  '': strval($variable);
														break;
							}
					}
				} elseif ($arr_key == 'c') {
