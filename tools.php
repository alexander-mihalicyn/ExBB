...
function showOnline() {
		...
		foreach ($onlinedata as $id => $online) {
				if ($fm->exbb['visiblemode'] === TRUE && !defined('IS_ADMIN') && $online['v'] === TRUE) continue;
				if ($online['pf'] !== FALSE && !isset($fm->user['private'][$online['pf']]) && !defined('IS_ADMIN')) $online['in'] = $fm->LANG['BoardMain'];
...
function banmemberslist() {
	...
	foreach ($banlist_keys AS $key => $user_id) {
		...
		if ( $fm->_CheckBanMember($user_id) ) continue;
	...
	while (false !== ($file = $d->read())) {
		if ( preg_match("#^([0-9]+)\.php$#is", $file, $matches)) {
			...
			if( ( ( isset($user['permanently']) && $user['permanently'] === false ) || !isset($user['permanently']) ) && $user['end'] < $time_end && !$is_moder ) continue;
...
function memberslist() {
		global $fm;
if (!$fm->user['id']) $fm->_Message($fm->LANG['MainMsg'], "Гости не могут просматривать список пользователей. 
<br>
<br>
