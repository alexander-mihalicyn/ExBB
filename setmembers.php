	...
	$fm->_Message($fm->LANG['UserAdmin'],$fm->LANG['UserCountUpd'].$total,'setmembers.php',1);
		...
		if ($fm->_String('checkaction') === 'yes' && $fm->_POST === TRUE) {
			...
			if ($newname === TRUE || $newemail === TRUE  || $newpassword  === TRUE) {
				...
				$emailtext 		= sprintf ( $fm->LANG['EmailNewPassName'],
			...
			$fm->_Message($fm->LANG['UserAdmin'],$fm->LANG['UserUpdatedOk'],'setmembers.php?action=edit_user&amp;userid='.$user_id,1);
				...
				foreach ($forums as $id=>$infa) {
						if ($infa['private']) {
							if (stristr($infa['catid'], 'f')) continue;
				...
				if (empty($private)) $private = $fm->LANG['NoPrivateForums'];
				/* День рождения */
				$includemode = 'edit';
		...
		if ($fm->_String('DelLog') != ''
			&& $fm->_POST === TRUE
			&& ($log_name = $fm->_Intval('log_name')) !== 0
			&& preg_match("#^[0-9]{10}$#is",$log_name)
			&& file_exists(FM_LOGDIR.$log_name.'.php')) {
        ...
        $logdata = preg_replace(array("#\(ad\) :: #is", "#\(mo\) :: #is"), array("(<font color=\"red\">{$fm->LANG['LogAdmin']}</font>) :: ", 
			"(<font color=\"#0000ff\">{$fm->LANG['Moderation']}</font>) :: "),$logdata);
