...
class Watches {
	...
	function watchingForums($forums, $lasts) {
		...
		foreach ($forums as $forum => $topics) {
			...
			$forums[$forum] = array( count($forums[$forum]), ( $lasts[$forum] && isset( $forums[$forum][$lasts[$forum]] ) ) );
	...
	function watchingTopics($topics) {
		...
		foreach ($topics as $forum => $list) {
			foreach ($list as $topic => $info) {
				if (!is_bool($info)) {
					$topics[$forum][$topic] = ( $topics[$forum][$topic]['postdate'] > max($deadlines[0], $deadlines[$forum]) && $topics[$forum][$topic]['p_id'] != $fm->user['id'] );
				}
			}
		}
	...
	function markForums($forums = array()) {
		...
		if (!$forums) {
			$sql = 'DELETE FROM watches;' . "INSERT INTO watches VALUES (0, 0, {$fm->_Nowtime});";
			$this->_execSqlite($sql);
	...
	function upDeadlines($uid = 0) {
		...
		$sql = "DELETE FROM watches WHERE time <= {$newDeadline};" . "UPDATE watches SET time = {$newDeadline} WHERE forum = 0 AND topic = 0;";
		$this->_execSqlite($sql);
	...
	function _openSqlite($uid = 0) {
		...
		if (!$exists) {
			$sql = 'CREATE TABLE watches (forum INTEGER, topic INTEGER, time INTEGER, PRIMARY KEY (forum, topic));' . 'CREATE INDEX time ON watches (time);' . "INSERT INTO watches VALUES (0, 0, {$fm->_Nowtime});";
			$this->_execSqlite($sql);
		}
