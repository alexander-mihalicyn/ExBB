...
if (!empty($request->query['r'])) {
	...
	$controllerPath = EXBB_ROOT.'/Controllers/'.ucfirst($controller).'Controller.php';

	if (file_exists($controllerPath)) {
		require EXBB_ROOT.'/Controllers/BaseController.php';

		include $controllerPath;

		$className = '\\Forum\\Controllers\\'.ucfirst($controller).'Controller';
...
$allforums = array_filter($fm->_Read(EXBB_DATA_FORUMS_LIST), function ($var) use ($fm) {
	...
	return ( isset( $fm->user['private'][$var['id']] ) && $fm->user['private'][$var['id']] );
});
	...
	if (isset( $subforums[$id] )) {
		foreach ($subforums[$id] as $subid => $subname) {
				...
				if (!$fm->exbb['watches'] && $fm->user['id'] !== 0) {
					...
					if ($allforums[$subid]['last_time'] > $fm->user['last_visit']) {
						...
						if (count($alltopic) > 0) {
							...
							$id = $did;
							$totalnew = count($alltopic);
							$UnreadFlag = $totalnew > 0;
							$newposts += $totalnew;
						}
			...
			if ($fm->user['id'] !== 0 && isset( $allforums[$subid]['last_time'] )) {
				$folderpicture = $UnreadFlag ? $yes_forumicon : $no_forumicon;
			}
			else {
...
function today(&$todayvisit) {
	...
	foreach ($today['members'] as $id => $member) {
		...
		if ($member['v']) {
			$hiddens++;
			if (defined('IS_ADMIN') || $id == $fm->user['id']) {
				$member['n'] .= '*';
			}
			else {
				continue;
	...
	$todayvisit = sprintf($fm->LANG['TodayVisit'], $today['guests'] + $members + $hiddens, $today['guests'], $members, ( $hiddens ) ? sprintf($fm->LANG['HiddensOnline'], $hiddens) : '', ( $was ) ? ' <span id="sp_todayvisit">(<a href="#" onClick="spoiler(\'_todayvisit\'); return false;">' . $fm->LANG['SpoilerShow'] . '</a>)</span><div id="spoiler_todayvisit" style="display: none;">
