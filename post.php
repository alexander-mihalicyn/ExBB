...
switch ($fm->input['action']) {
	case 'new'			:	newthread();
							break;
	case 'addnew'		: 	switch ($fm->_String('preview')) {
								case ''	:	addnewthread();
											break;
								default	:	newthread();
											break;
	...
	case 'reply'		:	reply();
							break;
	case 'addreply'		:	switch ($fm->_String('preview')) {
								case ''	:	addreply();
											break;
								default	:	reply();
											break;
...
function addnewthread() {
        ...
        $attach = FALSE;
		if ($upload !== 0 && !empty($_FILES['FILE_UPLOAD']) && ($attach = $fm->Upload($allforums[$forum_id]['upload'],
													uniqid("att-".$forum_id."-"),
													'uploads/',
        											'file')) !== FALSE) {
			if (defined("UP_ERROR")) {
        ...
        while (file_exists('forum'.$forum_id.'/'.$topic_id.'-thd.php')) $topic_id++;
		...
		else unset($allforums[$forum_id]['last_sub']);
		...
		if ($fm->exbb['emailfunctions'] === TRUE && $fm->exbb['mail_posts'] === TRUE) {
			...
			$email = sprintf($fm->LANG['NewPostThanks'],
							$fm->user['name'],
			...
			if (isset($emailers[$fm->user['id']])) unset($emailers[$fm->user['id']]);

			if (count($emailers)) {
				$email = sprintf($fm->LANG['NewTopicNotify'],
							$fm->user['name'],
...
function addreply() {
		...
		if ($fm->user['id'] !== 0 && $topic[$last_key]['p_id'] == $fm->user['id'] && ($fm->_Nowtime - $last_key) < FM_SUBPOST_TIME && $attach === FALSE) {
			...
			$PostAdded = FALSE;
				...
				if ($continueTopic = filesize('forum'.$forum_id.'/'.$topic_id.'-thd.php') >= FM_MAX_THREAD_SIZE ) {
					...
					while (file_exists($check_file.$newtopic_id.'-thd.php')) $newtopic_id++;
		...
		$pforum = (stristr($pcatid, 'f')) ? substr($pcatid, 1, strlen($pcatid) - 1) : 0;
		if ($pforum) {
			if ($PostAdded) $allforums[$pforum]['posts']++;
			$allforums[$pforum]['last_poster']		= $fm->user['name'];
			$allforums[$pforum]['last_poster_id']	= $fm->user['id'];
		...
		if (isset($list[$topic_id]['views'])) unset($list[$topic_id]['views']);
