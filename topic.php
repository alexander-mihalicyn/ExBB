...
switch ($allforums[$forum_id]['strep']) {
	case 'all':		$access = TRUE;
					break;
	case 'reged':	$access = ($fm->user['id'] !== 0) ? TRUE:FALSE;
					break;
	case 'admo':	$access = ($fm->_Moderator === TRUE) ? TRUE:FALSE;
					break;
}
...
foreach ($_pinmsg as $offset => $key) {
	if (!isset($threads[$key])) unset($_pinmsg[$offset]);
	else unset($threads_keys[array_search($key, $threads_keys)]);
}
...
else if ($fm->input['v'] == 'u' && $fm->user['id'] !== 0) {
...
if ($fm->exbb['statvisit']) {
	if ($statvisit['topic']) viewing($stattopic);
}
...
foreach ($threads_keys as $id => $key) {
		...
		if (isset($threads[$key]['attach_id']) && isset($t_attaches[$threads[$key]['attach_id']])) {
			...
			if ($t_attaches[$attach_ID]['type'] === 'image' && $fm->exbb['show_img'] === TRUE) {
				if ($fm->exbb['imgpreview'] === TRUE) {
					$post .= ($t_attaches[$attach_ID]['width'] > 250) ? $fm->LANG['ImgAttachTmb'].'<a href="printfile.php?action=attach&img=yes&f='.$forum_id.'&t='.$topic_id.'&id='.$attach_ID.'" title="'.$fm->LANG['ToIncrease'].'" rel="clearbox"><img src="printfile.php?action=attach&icon=yes&img=yes&f='.$forum_id.'&t='.$topic_id.'&id='.$attach_ID.'" alt="'.$attach_name.'" style="border: 1px outset #DCDCDC;"></a><br>':
		...
		else $pinmsg = (in_array($key, $_pinmsg)) ? '<img src="templates/'.DEF_SKIN.'/im/pinned.gif" border="0">' : '';
...
if ($topic['poll']) $poll_html = poll($forum_id,$topic_id);
unset($users);
...
function setup_guest($user_id = 0){
		...
		$users[$user_id] = array('user'			=> $fm->LANG['Guest'],
								'usertitle'		=> ($user_id === 0) ? $fm->LANG['NoReged']: $fm->LANG['UserDeleted'],
								'team'			=> '',	'useravatar'	=> '',	'usergraphic'	=> '',
								'onl'			=> '',	'posts'			=> '',	'jnd'			=> '',
								'location'		=> '',	'prf'			=> '',	'eml'			=> '',
								'www'			=> '',	'icq'			=> '',	'uin'			=> '',
								'aim'			=> '',	'pm'			=> '',	'signature'		=> '',
...
function setup_member($user_id) {
		...
		$users[$user_id] = array('user'			=> $user['name'],
								...
								'usergraphic'	=> '',
								'onl'			=> '',
								'posts'			=> sprintf($fm->LANG['UsertotalPosts'],$user['posts']),
								'jnd'			=> $fm->LANG['UserRegDate'].' <b>'.(($user['joined']) ? $fm->_JoinDate($user['joined']) : $fm->LANG['NA']).'</b> &nbsp;',
								'location'		=> ($fm->exbb['location'] === TRUE && $user['location'] != '') ? '<br>'.$fm->LANG['From'].': '.$user['location']:'',
								'prf'			=> '<a href="profile.php?action=show&member='.$user_id.'" title="'.$fm->LANG['UserProfile'].' '.$user['name'].'" target="_blank">'.$_icon['prf'].'</a>'.$_icon['divider'],
								'eml'			=> '',
								'www'			=> ($user['www'] !== '') ? '<a href="'.$user['www'].'" target="_blank">'.$_icon['www'].'</a>'.$_icon['divider']:'',
								'icq'			=> ($user['icq'] != '') ? '<a href="'.(($fm->exbb['redirect']) ? $fm->out_redir : '').'http://people.icq.com/'.$user['icq'].'">'.sprintf($_icon['icq'],$user['icq']).'</a> '.$_icon['divider']:'',
								'uin'			=> ($user['icq'] != '') ? $user['icq']:'',
								'aim'			=> ($user['aim'] != '') ? '<a href="aim:goim?screenname='.$user['aim'].'&amp;message=Hello+Are+you+there?">'.$_icon['aol'].'</a>'.$_icon['divider']:'',
								'pm'			=> ($fm->exbb['pm'] === TRUE && $fm->user['id'] !== 0) ? '<a href="messenger.php?action=new&touser='.$user_id.'" title="'.$fm->LANG['SendPm'].' '.$user['name'].'" target="_blank">'.$_icon['pm'].'</a>':'',
								'signature'		=> ($fm->exbb['sig'] === TRUE && $user['sig_on'] === TRUE && $user['sig'] != '') ? '<br><br>-----<br>'.$user['sig']:'',
								'karma'			=> '',
								'karmalink'		=> '',
								'pun'			=> '',
								'addpun'		=> ''
...
function viewing(&$viewing) {
	...
	foreach ($onlinedata as $sess => $online) {
		if (preg_match("#\"topic\.php\?forum=".$forum_id."\&topic=".$topic_id."\"#is", $online['in'])) {
			if (!$online['id']) $guests++;
			elseif ($fm->exbb['visiblemode'] && !empty($online['v']) && !defined('IS_ADMIN')) $hiddens++;
			else {
				if (empty($online['v'])) $members++;
				else {
					$hiddens++;
					$online['n'] .= '*';
				}
