...
switch ($fm->input['action']) {
	case 'edit'			:	editform();
							break;
	case 'processedit'	:	if ($fm->_Boolean($fm->input,'deletepost') === TRUE) {
								deletepost();
							} else {
							...
							break;
	case 'edittopic'	:	edit_topic_title();
							break;
	...
	case 'poll'			:	poll_edit();
							break;
	case 'lock'			:
	case 'unlock'		:	un_lockthread($fm->input['action']);
							break;
	case 'pin'			:
	case 'unpin'		:	un_pinthread($fm->input['action']);
							break;
	case 'pinmsg'		:	pinmsg();
							break;
	case 'trackers'		:	del_subscribed();
							break;
	case 'delselected'	:	delselected();
							break;
	case 'innew'		:	innew();
							break;
	case 'inexists'		:	inexists();
							break;
...
$fm->_Message($fm->LANG['MessageEdit'], $fm->LANG['EditTime']);
		...
		$upload	= ($fm->exbb['file_upload'] === TRUE && $allforums[$forum_id]['upload'] !== 0 && ($fm->user['upload'] === TRUE || $fm->exbb['autoup'] === TRUE && $fm->user['id'])) ? $allforums[$forum_id]['upload'] : 0;
		$attach = FALSE;
		if ($upload !== 0 && !empty($_FILES['FILE_UPLOAD']) && !preg_match("#^(del|keep)$#is",$fm->_String('editattach'))
			&& ($attach = $fm->Upload($upload,
									uniqid("att-".$forum_id."-"),
									'uploads/',
									'file')) !== FALSE) {
			if (defined("UP_ERROR")) {
...
function deletepost() {
		...
		if ($poster_id !== 0 && $fm->_Checkuser($poster_id)) {
			...
			if ($user['posted'][$forum_id] <= 0) unset($user['posted'][$forum_id]);
...
function deletethread() {
		...
		if ($fm->_Boolean($fm->input,'request') === TRUE) {
			$fm->_SAVE_STATS(array ('totalthreads' => array(1, -1),
									'totalposts' => array($list[$topic_id]['posts'], -1)));
            ...
            if (count($users) !== 0) UpdateAutorsInfoDelete($users,$forum_id);
            ...
            foreach ($attaches as $id => $attach) {
            		if (file_exists('uploads/'.$attach['id'])) unlink('uploads/'.$attach['id']);
            }
            @unlink('forum'.$forum_id.'/attaches-'.$topic_id.'.php');
			...
			$fm->_Write($fp_t_track,$_t_track);
           	...
           	if (file_exists('forum'.$forum_id.'/'.$topic_id.'-poll.php')) unlink('forum'.$forum_id.'/'.$topic_id.'-poll.php');
...
function movetopic() {
		...
		if ($fm->_Boolean($fm->input,'request') === TRUE) {
			...
			while (file_exists('forum'.$forum_id.'/'.$newtopic_id.'-thd.php')) $newtopic_id++;
			...
			$fm->_WriteLog(sprintf($fm->LANG['MoveTopicLog'], $tolist[$newtopic_id]['name'], strip_tags($allforums[$forum_id]['name']),
				strip_tags($allforums[$toforum_id]['name'])), 2);
...
if (!$fm->_POST) {
  ...
  $pollansw = explode("\n", $pollansw);
  if ((!$pollansw[0]) || (count($pollansw) > $fm->exbb['max_poll'])) $error[] = sprintf($fm->LANG['PollError'], $fm->exbb['max_poll']);
...
function poll_edit() {
		...
		if ($fm->_Boolean($fm->input,'savepoll') === TRUE) {
			if ($fm->_Boolean($fm->input,'delpoll') === TRUE) {
				...
				$pollansw	= '';
				foreach ($poll_data['choices'] as $choice)  $pollansw .= $choice[1]."\n";
...
function un_lockthread($mode) {
		...
		$fm->_WriteLog(sprintf($mode == 'lock' ? $fm->LANG['CloseTopicLog'] : $fm->LANG['OpenTopicLog'], $list[$topic_id]['name'],
			strip_tags($allforums[$forum_id]['name'])), 2);
...
function un_pinthread($mode) {
		...
		$fm->_WriteLog(sprintf($mode == 'pin' ? $fm->LANG['PinTopicLog'] : $fm->LANG['UnpinTopicLog'], $list[$topic_id]['name'],
			strip_tags($allforums[$forum_id]['name'])), 2);
...
function pinmsg() {
	...
	if ($fm->_Moderator === FALSE) $fm->_Message($fm->LANG['MsgPin'], $fm->LANG['EditNo']);
	...
	if (!isset($threads[$firstkey]['pinmsg'])) $threads[$firstkey]['pinmsg'] = array();
...
function delselected() {
		...
		$fm->_WriteLog($countremoved == 1 ? sprintf($fm->LANG['DeletePostLog'], $list[$topic_id]['name'], strip_tags($allforums[$forum_id]['name'])) : 
			sprintf($fm->LANG['DeletePostsLog'], $countremoved, $list[$topic_id]['name'], strip_tags($allforums[$forum_id]['name'])), 2);
		...
		if (count($attaches) > 0) {
			...
			foreach ($attaches as $attach_id) {
					if (isset($allattaches[$attach_id])){
						if (file_exists('uploads/'.$allattaches[$attach_id]['id'])) unlink('uploads/'.$allattaches[$attach_id]['id']);
						unset($allattaches[$attach_id]);
					}
			}
			$fm->_Write($fp_attach,$allattaches);
			if (count($allattaches) === 0) unlink('forum'.$forum_id.'/attaches-'.$topic_id.'.php');
		}

		if (count($autors) !== 0) UpdateAutorsInfoDelete($autors,$forum_id);
...
function innew() {
        ...
        if ($fm->_Boolean($fm->input,'moving') === TRUE) {
            ...
            while(file_exists('forum'.$toforum_id.'/'.$newtopic_id.'-thd.php')) $newtopic_id++;
            ...
            $fm->_SAVE_STATS(array ('totalposts' => array(1, -1),
            						'totalthreads' => array(1, 1)));
			include('modules/belong/_inNew.php');
			// Оставим запись в логе о перемещении сообщений в новую тему
			$fm->_WriteLog(sprintf($fm->LANG['InNewLog'], $countmoving, $list[$topic_id]['name'], strip_tags($allforums[$forum_id]['name']),
				$fm->input['topictitle'], strip_tags($allforums[$toforum_id]['name'])), 2);
...
$newlastposter = GetName($newtopic[$newlastkey]['p_id']);
			...
			$fm->_WriteLog(sprintf($fm->LANG['InExistsLog'], $countmoving, $list[$topic_id]['name'], strip_tags($allforums[$forum_id]['name']),
				@$newtopicname, strip_tags($allforums[$toforum_id]['name'])), 2);
        ...
        if ($toforum !== 0){
            ...
            $jumphtml .= $allforums[$toforum]['name'];
				...
				foreach ($allforums as $forumid => $forum){
						if (!defined('IS_ADMIN') && $forum['private'] === TRUE && !isset($fm->user['private'][$forumid])) continue;
						if (stristr($forum['catid'], 'f')) continue;
...
function JumpTopicHTML($toforum_id,$forum_id,$topic_id) {
		...
		foreach ($list as $top_id => $topicinfo){
				if ($top_id == $topic_id && $toforum_id == $forum_id) continue;
