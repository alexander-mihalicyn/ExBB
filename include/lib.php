...
function php($code) {
		global $fm,$html;
		if ($html === FALSE) $code[1] = $fm->html_replace($code[1]);
		$code = trim($code[1]);
...
function topic_icon($topic, $read_time=-1, $watched = null) {
		...
		if ($topic['pinned'] === TRUE && ($watched === null && $topic['postdate'] <= $read_time || !$watched)) return '<img src="'.$icon_path.'/sticky.gif" border="0">';
		if ($topic['pinned'] === TRUE && ($watched === null && $topic['postdate'] > $read_time || $watched)) return '<img src="'.$icon_path.'/stickynew.gif" border="0">';
		...
		if ($topic['posts'] >= $fm->exbb['hot_topic'] && ($watched === null && $topic['postdate'] <= $read_time || !$watched)) return '<img src="'.$icon_path.'/hotnonew.gif" border="0">';
		if ($topic['posts'] >= $fm->exbb['hot_topic']) return '<img src="'.$icon_path.'/hotnew.gif" border="0">';
		if ($watched === null && $topic['postdate'] > $read_time || $watched) return '<img src="'.$icon_path.'/new.gif" border="0">';
...
function CheckForumPerms($permission,$mode) {
		global $fm;
		switch ($permission) {
			case 'admo':	if ($fm->_Moderator === FALSE) {
								$fm->_FcloseAll();
								$fm->_Message($fm->LANG['MainMsg'],$fm->LANG[$mode.'admo']);
							}
							break;
       		case 'reged':	if ($fm->user['id'] === 0) {
								$fm->_FcloseAll();
...
function add_attach ($attach,$forumid,$toipcid,$attach_oldid = 0,$mode = 'new') {
        ...
        if (count($attachdata) == 0) unlink('forum'.$forumid.'/attaches-'.$toipcid.'.php');
        return $return;
