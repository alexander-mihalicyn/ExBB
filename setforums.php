...
if ($fm->input['action'] == "addcat" || $fm->input['action'] == "addforum") {
	...
	$allforums	= $fm->_Read(FM_ALLFORUMS,FALSE);
	...
	include('./admin/footer.tpl');
		...
		$dirtomake = 'forum'.$forum_id;
		if (!ini_get('safe_mode') && !is_dir($dirtomake)) {
			mkdir($dirtomake,$fm->exbb['ch_dirs']);
			@chmod($dirtomake,$fm->exbb['ch_dirs']);
		} else {
				if (!is_dir($dirtomake)) {
					$fm->_Fclose($fp_allforums);
					$fm->_Message($MessageTitle,sprintf($fm->LANG['SafeModeCat'],'forum'.$forum_id),'',1);
				}
		}
		...
		copy(FM_ALLFORUMS,FM_ALLFORUMS_BAK);
		@chmod(FM_ALLFORUMS_BAK,$fm->exbb['ch_files']);
		...
		$redir = 'setforums.php';
		if (stristr($catid, 'f')) $redir .= '?subforum='.substr($catid, 1, strlen($catid) - 1);
		...
		if ($fm->_String('doedit') != '') {
			...
			foreach ($allforums as $id=>$forum){
					if ($catid == $forum['catid']) $allforums[$id]['catname'] = $fm->html_replace($fm->input['catname']);
			}
			$fm->_Write($fp_allforums,$allforums);
		...
		switch ($fm->input['action']) {
				case 'delcat':		$forum_id 		= $fm->_String('catid');
									...
									break;
				case 'delete':		$forum_id 		= $fm->_Intval('forum');
									...
									$RequestText 	= $fm->LANG['RequestYourAction'];
									break;
				case 'moveforum':	$forum_id 		= $fm->_Intval('forum');
		...
		$allforums	= $foreach = $fm->_Read2Write($fp_allforums,FM_ALLFORUMS,FALSE);
        array_filter($allforums,'GET_CATID');
		...
		if (!isset($allforums[$forum_id])) {
			...
			foreach ($allforums as $id => $forum) {
				if ($forum['catid'] == 'f'.$forum_id || $forum_id == $id) {
					if ($allforums[$id]['private'] === TRUE) $private[$id] = 1;
		...
		$fm->_SAVE_STATS(array('totalposts' => array($d_posts, -1),
								'totalthreads' => array($d_topics, -1)));
		...
		$fm->_BOARDSTATS();
		$fm->LANG['NowStat'] = sprintf( $fm->LANG['NowStat'],
										$fm->_Stats['totalposts'],
										$fm->_Stats['totalthreads'],
										$totalposts,
										$totalthreads);
		...
		if ($topiccount > 0) {
			foreach ($list as $id => $topic) $postscount += $topic['posts'];
        ...
        $views_data	= $fm->_Read2Write($fp_views, 'forum'.$forum_id.'/views.php');
        $fm->_Read2Write($fp_list,'forum'.$forum_id.'/list.php');
		...
		if ($subforum) $fm->LANG['CatAddNew'] = $fm->LANG['ForumAddNew'] = $fm->LANG['AddNewSub'];
		$allforums	= $fm->_Read(FM_ALLFORUMS,FALSE);
        //prints($allforums);
		$lastcategoryplace = -1;
		foreach ($allforums as $forumid => $forum) {
			if ((stristr($forum['catid'], 'f')) && (!$subforum)) {
				...
				if (!isset($subforums[$pid])) $subforums[$pid] = 1;
				else $subforums[$pid]++;
				continue;
			}
			if (($subforum) && ($forum['catid'] != 'f'.$subforum)) continue;
			if (($subforum) && ($forum['catid'] == 'f'.$subforum)) $forum['catname'] = '&lt;- '.$allforums[$subforum]['name'];
...
function categories($allcats,$catid, $move = FALSE, $forum_id = 0) {
	global $fm;
		...
		$lastcat = -1;
		foreach ($allforums as $id => $forum) {
				if ($ch = stristr($forum['catid'], 'f')) $sub[] = substr($forum['catid'], 1, strlen($forum['catid']) - 1);
				if (/*($move === TRUE && $forum['catid'] == $catid) ||*/ ($ch)) continue;
				$selected = ($forum['catid'] == $catid) ? ' selected' : '';
				if ($lastcat != $forum['catid']) $cathtml .= '<option value="'.$forum['catid'].'"'.$selected.'>'.$forum['catname']."</option>\n";
				$selected = ($id == substr($catid, 1, strlen($catid) - 1)) ? ' selected' : '';
				if ((($move === TRUE) && (stristr($catid, 'f')) && ($id != substr($catid, 1, strlen($catid) - 1))) || ($move === TRUE && !stristr($catid, 'f') && !in_array($forum_id, $sub) && $forum_id != $id) || ($move === FALSE)) $cathtml .= '<option value="f'.$id.'"'.$selected.'>-- '.$forum['name'].'</a>';
				$lastcat = $forum['catid'];
		}
		$cathtml .= '</select>';
		return $cathtml;
