...
function search() {
		...
		if ($fm->input['action'] === '') {
			...
			include('./templates/'.DEF_SKIN.'/footer.tpl');
				...
				preg_match_all('/([^a-zA-Zà-ÿÀ-ß¸¨]|^)([a-zA-Zà-ÿÀ-ß¸¨]{4,})(?![a-zA-Zà-ÿÀ-ß¸¨])/',$fm->input['search_keywords'],$key_words);
				...
				$t_visits = $fm->_GetCookieArray('t_visits');
				$found = $_found = 0;
				$entered_word = preg_replace('/([^a-zA-Zà-ÿÀ-ß¸¨]|^)([a-zA-Zà-ÿÀ-ß¸¨]{1,3})(?![a-zA-Zà-ÿÀ-ß¸¨])/', '', $_SEARCH['entered_word']);
				...
				$searchinmessage = sprintf($fm->LANG['YOUSEARCH'],preg_replace('/([^a-zA-Zà-ÿÀ-ß¸¨]|^)([a-zA-Zà-ÿÀ-ß¸¨]{1,3})(?![a-zA-Zà-ÿÀ-ß¸¨])/', '$1<font color="red">$2</font>', $_SEARCH['entered_word']));
