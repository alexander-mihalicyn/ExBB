...
if ($fm->input['action'] == 'inbox') {
	...
	include('./templates/'.DEF_SKIN.'/footer.tpl');
		...
		if ($inboxdata[$message_id]['status'] === FALSE) {
			...
			if (count($senderoutbox)==0) unlink('messages/'.$sender_id.'-out.php');
			unset($senderoutbox);
		} else {
				$fm->_Fclose($fp_inbox);
		}
		...
		if (($message_id = $fm->_Intval('msg')) === 0 || !isset($outboxdata[$message_id])) {
			if (count($outboxdata) <= 0) unlink('messages/'.$fm->user['id'].'-out.php');
		...
		if ($fm->input['action'] == 'reply' || $fm->input['action'] == 'replyquote') {
			...
			if (($message_id = $fm->_Intval('msg')) === 0 || !isset($inboxdata[$message_id])) {
				$fm->_Fclose($fp_inbox);
				if (count($inboxdata) <= 0) unlink('messages/'.$fm->user['id'].'-msg.php');
		...
		switch ($fm->input['where']){
            case  'inbox': $fromfile = 'messages/'.$fm->user['id'].'-msg.php';break;
            case 'outbox': $fromfile = 'messages/'.$fm->user['id'].'-out.php';break;
        }
